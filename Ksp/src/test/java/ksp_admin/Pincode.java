...
public class Pincode extends Utilies {
	...
	@FindBy(xpath = "//input[@id='Pincode_Code']")
	WebElement codeField;

	@FindBy(xpath = "//select[@id='Pincode_OrderIndex']")
	WebElement orderIndexDropdown;

	@FindBy(xpath = "//select[@id='Pincode_StatusCode']")
	...
	@FindBy(id = "PincodeDTO_Code")
	WebElement Editcode;

	@FindBy(id = "PincodeDTO_OrderIndex")
	WebElement Editorderindex;

	@FindBy(id = "PincodeDTO_StatusCode")
	...
	public Pincode(WebDriver driver) throws AWTException {
		this.driver = driver;
	...
	public void openpincode() {
		wait.until(ExpectedConditions.elementToBeClickable(By.linkText("Pincode"))).click();
		sleep(1000);
	}
	...
	public void addd() {
		...
		try {
			FileInputStream fis1 = new FileInputStream("D://KSP_Admin//Pincode.xlsx");// "D:\steno\TestDataAPC.xlsx"
			...
			for (int i = 1; i <= 10; i++) {
				...
				int orderIndex = (int) row.getCell(2).getNumericCellValue();
				String Status = getCellValue(row.getCell(3));

				String codeEdit = getCellValue(row.getCell(5));
				int orderIndexEdit = (int) row.getCell(6).getNumericCellValue();
				String statusEdit = getCellValue(row.getCell(7));
				int entriesperpage = (int) row.getCell(8).getNumericCellValue();

				double doubleValue = Double.parseDouble(codeEdit.trim()); // Handles "624445.0"
				int intValue = (int) doubleValue; // Converts
				// Convert the integer back to String for sendKeys
				String intAsString = String.valueOf(intValue);

				double doubleValue1 = Double.parseDouble(code.trim()); // Handles "624445.0"
				int intValue1 = (int) doubleValue1; // Converts
				// Convert the integer back to String for sendKeys
				...
				codeField.sendKeys(intAsString);
				...
				if (!isElementClickable(driver, entriesperpages)) {
					try {
						...
						String[] errorIds = { "pincode_Code-error", "pincode_OrderIndex-error","pincode_StatusCode-error" };
				...
				searchBar.sendKeys(intAsString);
				...
				if (isElementClickable(driver, viewButton)) {
					...
					row1.createCell(2).setCellValue(getOrderIndex);
					row1.createCell(3).setCellValue(getStatus);
					...
					Editcode.clear();
					Editcode.sendKeys(intAsString1);
					sleep(1000);
		...
		if (isElementClickable(driver, CancelEdit)) {
			try {
				try {
					boolean hasError = true;
					// List of specific error IDs (extend this as needed)
					String[] errorIds = { "PincodeDTO_Code-error" };
				...
				ok.click();
	            	...
	            	sleep(1000);
                    clickonElement(searchBar);
					searchBar.sendKeys(intAsString1);
					robot.keyPress(KeyEvent.VK_ENTER);
					robot.keyRelease(KeyEvent.VK_ENTER);
					sleep(500);
				...
				fileOut = new FileOutputStream("D://KSP_Admin//Pincode.xlsx");
