...
public class Kannada_paper extends Utilies {
	...
	@FindBy(id = "KannadaPaperType_OrderIndex")
	WebElement orderIndexDropdown;

	@FindBy(id = "KannadaPaperType_StatusCode")
	...
	@FindBy(id = "KannadaPaperTypeDTO_Code")
	WebElement Editcode;

	@FindBy(id = "KannadaPaperTypeDTO_Title")
	WebElement Edittitle;

	@FindBy(xpath = "//select[@id='KannadaPaperTypeDTO_OrderIndex' and @name='KannadaPaperTypeDTO.OrderIndex']")
	WebElement Editorderindex;

	@FindBy(xpath = "//select[@id='KannadaPaperTypeDTO_StatusCode' and @name='KannadaPaperTypeDTO.StatusCode']")
	...
	public Kannada_paper(WebDriver driver) throws AWTException {
		this.driver = driver;
	...
	public void openkannadapaperType() {
		wait.until(ExpectedConditions.elementToBeClickable(By.linkText("Kannada Paper Types"))).click();
		sleep(1000);
	}
	...
	public void addd() {
		...
		try {
			FileInputStream fis1 = new FileInputStream("D://KSP_Admin//Kannadapapertype.xlsx");// "D:\steno\TestDataAPC.xlsx"
			...
			for (int i = 1; i <= 10; i++) {
				...
				if (!isElementClickable(driver, entriesperpages)) {
					try {
						...
						String[] errorIds = { "KannadaPaperType_Code-error", "KannadaPaperType_Title-error","KannadaPaperType_OrderIndex-error", "KannadaPaperType_StatusCode-error" };
				...
				if (isElementClickable(driver, viewButton)) {
					...
					String getViewCode = getAttributeValue(By.name("KannadaPaperType.Code"));
					String getViewTitle = getAttributeValue(By.name("KannadaPaperType.Title"));
					String getOrderIndex = getAttributeValue(By.name("KannadaPaperType.OrderIndex"));
		...
		if (isElementClickable(driver, CancelEdit)) {
			try {
				try {
					boolean hasError = true;
					// List of specific error IDs (extend this as needed)
					String[] errorIds = { "KannadaPaperTypeDTO_Code-error","KannadaPaperTypeDTO_Title-error" };
				...
				fileOut = new FileOutputStream("D://KSP_Admin//Kannadapapertype.xlsx");
