...
public class Pincode_districts extends Utilies {
	...
	@FindBy(id = "PincodeDistrict_PinCode")
	WebElement pincodeField;

	@FindBy(id = "PincodeDistrict_DistrictCode")
	WebElement PincodeDistrictField;

	@FindBy(id = "PincodeDistrict_OrderIndex")
	WebElement orderIndexDropdown;

	@FindBy(id = "PincodeDistrict_StatusCode")
	...
	@FindBy(id = "PincodeDistrictDTO_PinCode")
	WebElement Editpincode;

	@FindBy(id = "PincodeDistrictDTO_DistrictCode")
	WebElement EditPincodeDistrict;

	@FindBy(id = "PincodeDistrictDTO_OrderIndex")
	WebElement Editorderindex;

	@FindBy(id = "PincodeDistrictDTO_StatusCode")
	...
	public Pincode_districts(WebDriver driver) throws AWTException {
		this.driver = driver;
	...
	public void openPincodedistricts() {
		wait.until(ExpectedConditions.elementToBeClickable(By.linkText("Pincode District"))).click();
		sleep(1000);
	}
	...
	public void addd() {
		...
		try {
			FileInputStream fis1 = new FileInputStream("D://KSP_Admin//Pincode_district.xlsx");// "D:\steno\TestDataAPC.xlsx"
			...
			for (int i = 1; i <= 10; i++) {
				...
				if (row == null) {
				 ...
				 int PincodeDistrict = (int)row.getCell(2).getNumericCellValue();
			    int orderIndex = (int) row.getCell(3).getNumericCellValue();
				String Status = getCellValue(row.getCell(4));

				 int codeEdit = (int) row.getCell(6).getNumericCellValue();
				 int PincodeDistrictEdit = (int) row.getCell(7).getNumericCellValue();
				...
				Select s = new Select(pincodeField);
				s.selectByIndex(code);
				new Select(PincodeDistrictField).selectByIndex(PincodeDistrict);
				...
				String pincodetext = s.getFirstSelectedOption().getText();
				clickonElement(save);
				sleep(2000);
				...
				searchBar.sendKeys(pincodetext);
				...
				if (isElementClickable(driver, viewButton)) {
					...
					Select s1 = new Select(Editpincode);
					s1.selectByIndex(codeEdit);

					new Select(EditPincodeDistrict).selectByIndex(PincodeDistrictEdit);
					sleep(1000);
					...
					String editpincodetext = s1.getFirstSelectedOption().getText();
		...
		if (isElementClickable(driver, CancelEdit)) {
			try {
				...
				ok.click();
	            	...
	            	sleep(1000);
                    clickonElement(searchBar);
					searchBar.sendKeys(editpincodetext);
					robot.keyPress(KeyEvent.VK_ENTER);
					robot.keyRelease(KeyEvent.VK_ENTER);
					sleep(500);
				...
				fileOut = new FileOutputStream("D://KSP_Admin//Pincode_district.xlsx");
