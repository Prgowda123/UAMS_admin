...
public class Sports_Games extends Utilies {
	...
	@FindBy(id = "SportAndGame_Code")
	WebElement codeField;

	@FindBy(id = "SportAndGame_Title")
	WebElement titleField;

	@FindBy(id = "SportAndGame_OrderIndex")
	WebElement orderIndexDropdown;

	@FindBy(id = "SportAndGame_StatusCode")
	...
	@FindBy(id = "SportAndGameDTO_Code")
	WebElement Editcode;

	@FindBy(id = "SportAndGameDTO_Title")
	WebElement Edittitle;

	@FindBy(id = "SportAndGameDTO_OrderIndex")
	WebElement Editorderindex;

	@FindBy(id = "SportAndGameDTO_StatusCode")
	...
	@FindBy(xpath = "//input[@type='checkbox' and @data-table='Sports&GamesTable']")
	...
	public Sports_Games(WebDriver driver) throws AWTException {
		this.driver = driver;
	...
	public void openSportsandgames() {
		wait.until(ExpectedConditions.elementToBeClickable(By.linkText("Sports / Games"))).click();
		sleep(1000);
	}
	...
	public void addd() {
		...
		try {
			FileInputStream fis1 = new FileInputStream("D://KSP_Admin//Games&games.xlsx");// "D:\steno\TestDataAPC.xlsx"
			...
			for (int i = 1; i <= 10; i++) {
				...
				if (!isElementClickable(driver, entriesperpages)) {
					try {
						...
						String[] errorIds = { "SportAndGame_Code-error", "SportAndGame_Title-error","SportAndGame_OrderIndex-error", "SportAndGame_StatusCode-error" };
		...
		if (isElementClickable(driver, CancelEdit)) {
			try {
				try {
					boolean hasError = true;
					// List of specific error IDs (extend this as needed)
					String[] errorIds = { "SportAndGameDTO_Code-error","SportAndGameDTO_Title-error" };
...
//				clickonElement(checkbox1);
				...
				fileOut = new FileOutputStream("D://KSP_Admin//Games&games.xlsx");
