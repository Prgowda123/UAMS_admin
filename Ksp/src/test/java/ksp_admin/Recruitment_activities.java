...
public class Recruitment_activities extends Utilies {
    ...
    @FindBy(id = "RecruitmentActivity_Code")
    WebElement codeField;

    @FindBy(id = "RecruitmentActivity_Title")
    WebElement titleField;

    @FindBy(id = "RecruitmentActivity_OrderIndex")
    WebElement orderIndexDropdown;

    @FindBy(id = "RecruitmentActivity_StatusCode")
    ...
    @FindBy(xpath = "//tr/td[contains(text(),'No data found')]")
    ...
    @FindBy(id="RecruitmentActivityDTO_Code")
    WebElement Editcode;

    @FindBy(id="RecruitmentActivityDTO_Title")
    WebElement Edittitle;

    @FindBy(xpath="//select[@id='OrderIndexDropdown' or @name='RecruitmentActivityDTO.OrderIndex']")
    WebElement Editorderindex;

    @FindBy(xpath="//select[@id='StatusDropdown' or @name='RecruitmentActivityDTO.StatusCode']")
    ...
    public Recruitment_activities(WebDriver driver) throws AWTException {
        this.driver = driver;
    ...
    public void openRecruitment_Activities() {
        wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[contains(text(),'Recruitment Activities')]"))).click();
        sleep(1000);
    }
    ...
    public void addd() {
        ...
        try
              {
            FileInputStream fis1 = new FileInputStream("D://KSP_Admin//Recruitment_activities.xlsx");//"D:\steno\TestDataAPC.xlsx"
            ...
            for (int i = 1; i <= 10; i++) {
				...
				if (!isElementClickable(driver, entriesperpages)) {
					try {
						...
						String[] errorIds = { "RecruitmentActivity_Code-error", "RecruitmentActivity_Title-error","RecruitmentActivity_OrderIndex-error", "RecruitmentActivity_StatusCode-error" };
				...
				if (isElementClickable(driver, viewButton)) {
					...
					String getViewCode = getAttributeValue(By.name("RecruitmentActivity.Code"));
					String getViewTitle = getAttributeValue(By.name("RecruitmentActivity.Title"));
					String getOrderIndex = getAttributeValue(By.name("RecruitmentActivity.OrderIndex"));
					String getStatus = getAttributeValue(By.xpath("//input[@id='statusTitle']"));
		...
		if (isElementClickable(driver, CancelEdit)) {
			try {
				try {
					boolean hasError = true;
					// List of specific error IDs (extend this as needed)
					String[] errorIds = { "RecruitmentActivityDTO_Code-error","RecruitmentActivityDTO_Title-error" };
				...
				fileOut = new FileOutputStream("D://KSP_Admin//Recruitment_activities.xlsx");
